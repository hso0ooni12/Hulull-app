...
const BookingRules={
 ...
 validateDate(date,today=this.today()){if(!this.allowed(date,today))throw new Error('الحجز من اليوم التالي فقط، والجمعة إجازة.');}
};
...
function bookingError(error){
 ...
 if(/BOOKING_/.test(m))return 'تعذر إرسال الطلب. حدّث الأيام المتاحة ثم حاول مرة أخرى.';
...
function hideCustomerTimeField(){
 ...
 time.required=false;time.value='';time.disabled=true;
...
async function loadBookingAvailability(){
 ...
 $('customerBookingSubmitBtn').disabled=true;
 $('bookingCalendarDays').querySelectorAll('button').forEach(b=>b.disabled=true);
 $('bookingAvailabilityMessage').textContent='جاري تحديث الأيام المتاحة...';$('bookingAvailabilityRetry').classList.add('hidden');
